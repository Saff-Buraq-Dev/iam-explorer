...
class TestIAMFetcher:
    ...
    def test_fetch_users_paginated(self):
        ...
        mock_client.list_users.side_effect = [
            {
                'Users': [{'UserName': 'alice', 'UserId': 'ID1', 'Arn': 'arn1', 'CreateDate': '2023-01-01T00:00:00Z', 'Path': '/'}],
                'IsTruncated': True,
                'Marker': 'marker1'
            },
            # Second page
            {
                'Users': [{'UserName': 'bob', 'UserId': 'ID2', 'Arn': 'arn2', 'CreateDate': '2023-01-01T00:00:00Z', 'Path': '/'}],
                'IsTruncated': False
            }
        ]
    ...
    def test_fetch_all_data_success(self):
        ...
        with patch.object(self.fetcher, 'fetch_users', return_value=[{'UserName': 'alice'}]):
            with patch.object(self.fetcher, 'fetch_roles', return_value=[{'RoleName': 'test-role'}]):
                with patch.object(self.fetcher, 'fetch_groups', return_value=[{'GroupName': 'developers'}]):
                    with patch.object(self.fetcher, 'fetch_policies', return_value=[{'PolicyName': 'CustomPolicy'}]):
                        with patch.object(self.fetcher, 'fetch_user_policies', return_value={'attached_policies': [], 'inline_policies': []}):
                            with patch.object(self.fetcher, 'fetch_role_policies', return_value={'attached_policies': [], 'inline_policies': []}):
                                with patch.object(self.fetcher, 'fetch_group_policies', return_value={'attached_policies': [], 'inline_policies': []}):
                                    with patch.object(self.fetcher, 'fetch_group_memberships', return_value=[]):
